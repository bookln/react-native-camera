...
- (void)stopSession
{
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    for (AVCaptureInput *input in self.session.inputs.reverseObjectEnumerator) {
      [self.session removeInput:input];
    }
    
    for (AVCaptureOutput *output in self.session.outputs.reverseObjectEnumerator) {
    ...
    if([metadata isKindOfClass:[AVMetadataMachineReadableCodeObject class]]) {
      ...
      for (id barcodeType in self.barCodeTypes) {
        if ([metadata.type isEqualToString:barcodeType]) {
          ...
          CGRect bounds = transformed != nil ? transformed.bounds : CGRectZero;
          id data = codeMetadata.stringValue != nil ? codeMetadata.stringValue : [NSNull null];
          ...
          NSDictionary *event = @{
            ...
            @"bounds": @{
                @"origin": @{
                    @"x": [NSString stringWithFormat:@"%f", bounds.origin.x],
                    @"y": [NSString stringWithFormat:@"%f", bounds.origin.y]
                },
                @"size": @{
                    @"height": [NSString stringWithFormat:@"%f", bounds.size.height],
                    @"width": [NSString stringWithFormat:@"%f", bounds.size.width]
                }
            }
          };
